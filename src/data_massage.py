...
def summarize_data(metadata, data):
    ...
    for ind in metadata:
        ...
        metadata[ind]["R_mean_mean"] = numpy.mean(induction["R_mean"])
        metadata[ind]["Temp_mean"] = numpy.mean(induction["Temp."])
        metadata[ind]["Humidity_mean"] = numpy.mean(induction["Humidity"])

        gas_max_time = induction.iloc[induction["R_mean"].argmax()]["timestamp"]

    return pandas.DataFrame.from_dict(metadata, orient="index")
...
def preprocess_data(metadata, data):
    ...
    # strangely, there is no induction 95 in the dataset, despite it being in the metadata
    # drop the key from the metadata, and everything else should fall into place
    metadata.drop([95], inplace=True)
    ...
    # average all the gas measurements together
